...
result=result+"/"
exclulists,pnglists = [],[]
exclupath = pathlib.Path(result).glob('2_1_*.png')
for e in exclupath:
...
for pr in range(len(pnglists)):
 ...
 if width/height <= 2/1:
  ...
  y_coordi = int((2160-width)/2)
  back_imga = Image.new('RGBA',(width, height),color=(0,0,0,0))
  back_img = Image.new('RGBA',(2160,1080),color=(0,0,0,255))
 ...
 if width/height > 2/1:
  ...
  if re_width > 1 :
     ...
     re_width = 2160/width
     re_height = math.ceil(height*re_width)
  re_img = img.resize((2160, re_height))
  width, height = re_img.size
 ...
 back_img.save(result+"2_1_"+pnglists[pr], quality=95)
if result !="/":
 sg.popup('変換が完了しました')