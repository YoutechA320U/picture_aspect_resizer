...
if result is None:
   result =""
exclulists,pnglists = [],[]
exclupath = pathlib.Path(result).glob('4_3_*.png')
for e in exclupath:
...
for pr in range(len(pnglists)):
 ...
 if width/height <= 4/3:
  ...
  y_coordi = int((1440-width)/2)
  back_imga = Image.new('RGBA',(width, height),color=(0,0,0,0))
  back_img = Image.new('RGBA',(1440,1080),color=(0,0,0,255))
 ...
 if width/height > 4/3:
  ...
  if re_width > 1 :
     ...
     re_width = 1440/width
     re_height = math.ceil(height*re_width)
  re_img = img.resize((1440, re_height))
  width, height = re_img.size
 ...
 back_img.save(result+"4_3_"+pnglists[pr], quality=95)
if result !="":
 sg.popup('変換が完了しました')